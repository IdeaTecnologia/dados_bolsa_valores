...
STATUSINVEST_INDICATORS_MAP = {
    ...
    "D.Y": "statusInvest_dy_percentual", "P/L": "statusInvest_pl", "PEG Ratio": "statusInvest_peg_ratio",
    "P/VP": "statusInvest_pvp", "EV/EBITDA": "statusInvest_ev_ebitda", "EV/EBIT": "statusInvest_ev_ebit",
    "P/EBITDA": "statusInvest_p_ebitda", "P/EBIT": "statusInvest_p_ebit", "VPA": "statusInvest_vpa",
    "P/Ativo": "statusInvest_p_ativo", "LPA": "statusInvest_lpa", "P/SR": "statusInvest_psr",
    "P/Cap. Giro": "statusInvest_p_cap_giro", "P/Ativo Circ. Liq.": "statusInvest_p_ativo_circ_liq",
    "Dív. líquida/PL": "statusInvest_div_liq_pl", "Dív. líquida/EBITDA": "statusInvest_div_liq_ebitda",
    "Dív. líquida/EBIT": "statusInvest_div_liq_ebit", "PL/Ativos": "statusInvest_pl_ativos",
    "Passivos/Ativos": "statusInvest_passivos_ativos", "Liq. corrente": "statusInvest_liq_corrente",
    "Giro ativos": "statusInvest_giro_ativos", "M. Bruta": "statusInvest_margem_bruta_percentual",
    "M. EBITDA": "statusInvest_margem_ebitda_percentual", "M. EBIT": "statusInvest_margem_ebit_percentual",
    "M. Líquida": "statusInvest_margem_liquida_percentual", "ROE": "statusInvest_roe_percentual",
    "ROA": "statusInvest_roa_percentual", "ROIC": "statusInvest_roic_percentual",
    "CAGR Receitas 5 anos": "statusInvest_cagr_rec_5anos_percentual",
    "CAGR Lucros 5 anos": "statusInvest_cagr_lucros_5anos_percentual",
    "Patrimônio líquido": "statusInvest_patrimonio_liquido", "Ativos": "statusInvest_ativos",
    "Ativo circulante": "statusInvest_ativo_circulante", "Dívida bruta": "statusInvest_divida_bruta",
    "Dívida líquida": "statusInvest_divida_liquida", "Valor de mercado": "statusInvest_valor_mercado",
    "Valor de firma": "statusInvest_valor_firma", "Nº total de papéis": "statusInvest_nro_papeis",
    "Free Float": "statusInvest_free_float_percentual", "Tag Along": "statusInvest_tag_along_percentual",
...
class StatusInvestScraper:
    ...
    def fetch_data(self):
        ...
        try:
            print(f"Iniciando scraper para {self.ticker} no StatusInvest com SeleniumBase...")
            
            # Adiciona argumentos para parecer mais humano e evitar detecção
            driver = Driver(
                uc=True, 
                headless=True, 
                agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36",
                block_images=True # Bloqueia imagens para carregar mais rápido
            )
            driver.get(self.url)

            # --- NOVA LÓGICA PARA LIDAR COM O CLOUDFLARE ---
            try:
                # Tenta encontrar o iframe do Cloudflare e clicar na caixa de verificação
                print("Procurando por desafio Cloudflare...")
                iframe_selector = 'iframe[title="Widget containing a Cloudflare security challenge"]'
                driver.wait_for_element(iframe_selector, timeout=10) # Espera no máximo 10s pelo desafio
                ...
                time.sleep(5) # Dá um tempo para o redirecionamento acontecer
                driver.switch_to.default_content() # Volta para o contexto principal da página
            except Exception:
                # Se não encontrar o iframe em 10 segundos, assume que não há desafio e continua
                print("Nenhum desafio Cloudflare encontrado. Prosseguindo...")
            
            # Espera inteligente pelo conteúdo real da página
            print("Aguardando carregamento da página principal do Status Invest...")
            driver.wait_for_element("#indicators-section", timeout=25)
            ...
            print(f"Erro principal no scraper para {self.ticker}: {e}")
            dados["erro_statusinvest"] = f"Status Invest: Falha na automação: {str(e)}"
            return dados
        
        finally:
            # Garante que o navegador seja sempre fechado ao final
            if driver:
                print(f"Fechando navegador para {self.ticker}.")
                driver.quit()